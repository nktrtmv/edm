...
using Edm.Document.Searcher.GenericSubdomain.Enrichers.Abstractions.Targets.Generics;

using Microsoft.Extensions.Logging;

namespace Edm.Document.Searcher.GenericSubdomain.Enrichers.Abstractions.Sources.Generics;
    ...
    where TKey : notnull
{
    ...
    protected void EnrichTargetModels(EnricherTargetGeneric<TKey, TValue>[] targets, Dictionary<TKey, TValue> values)
    {
        foreach (EnricherTargetGeneric<TKey, TValue> target in targets)
        {
            ...
            {
                string keys = string.Join(", ", values.Keys);
