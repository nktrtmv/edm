...
public sealed class ConditionExistsAny : ConditionBase
{
    static ConditionExistsAny()
    {
    ...
    public ConditionExistsAny(ConditionData data) : base(data)
    {
    }
    ...
    protected override ConditionResult OnCheck(DocumentAttributeValue[] attributesValues)
    {
        ...
        if (linkedAttributeValues.Any(CheckAttributeValue))
        {
            return ConditionResult.Succeed(Id);
        }

        Id<DocumentAttribute>[] notValid = linkedAttributeValues.Select(v => v.Id).ToArray();
